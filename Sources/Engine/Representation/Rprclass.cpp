 ...
 ***************************************************************/
/*
// $Log: /ILWIS 3.0/Representation/Rprclass.cpp $
 * 
 * 17    4-03-03 17:09 Retsios
 * Multiply default symbol size by iSIZE_FACTOR to compensate for the
 * reduction of its size by the same factor in MapWindow
 * 
 * 16    13-12-02 17:49 Retsios
 * Give another default line width than 0.0 (bug# 6357)
 * 
 * 15    2/07/02 8:17a Martin
 * Domain has become a associated file, to prevent problems with circular
 * dependencies (grrr!, who ever invented this). Linked tables in the same
 * directory will go wrong if this is not done
 * 
 * 14    10/30/01 16:59 Willem
 * Added the GetObjectStructure function now to be able to add the domain
 * reference for the [table] ODF section
 * 
 * 13    8/24/01 13:03 Willem
 * Removed the SetReadOnly() function. This is now handled by
 * IlwisObjectPtr::SetReadOnly() for all ilwis objects
 * 
 * 12    8/01/01 2:56p Martin
 * record is now in the Ilwis namepsace
 * 
 * 11    16-03-01 20:42 Koolhoven
 * added several tbl->DoNotStore(false) to ensure that opening as table
 * can edit the rpr
 * 
 * 10    10/20/00 11:12a Martin
 * sdefault second color is now white instead of black
 * 
 * 9     9/18/00 3:44p Martin
 * no stores form the internal table unless explicitly called for
 * 
 * 8     18/09/00 13:10 Willem
 * RepresentationClass forgot to also set the read/only flag for itself
 * 
 * 7     8-02-00 18:01 Wind
 * set fUpdateCatalog flag to false after creation of internal table
 * 
 * 6     29-10-99 9:20 Wind
 * thread save stuff
 * 
 * 5     9/24/99 10:38a Wind
 * replaced calls to static funcs ObjectInfo::ReadElement and WriteElement
 * by calls to member functions
 * 
 * 4     9/10/99 12:56p Wind
 * comments
 * 
 * 2     9/08/99 12:57p Wind
...
// 22beta2
//
// Revision 1.2  1997/08/18 13:54:34  Wim
// Default color for symbol fill corrected
//
/* RepresentationClass
   by Wim Koolhoven
  (c) Ilwis System Development ITC
	Last change:  WK   15 Jun 98    6:17 pm
*/
