...
{
   if (parent == 0)
      throw std::runtime_error("Trying to push an item above the root node of QuadTree.");
...
{
   if (isPartitioned)
      throw std::runtime_error("Trying to partition a branched node.");
...
void QuadTreeNode::removeItem(int iItem)
{
   if (iItem >= (int)items.size())
      throw std::runtime_error("Trying to remove an item inside QuadTreeNode, but the given index is out of bound.");
