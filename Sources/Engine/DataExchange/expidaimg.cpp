...
{
  trq.SetTitle(SCVTitleExportIDA);  // the title in the report window
  ...
  if (inMap->dm()->pdi())         // Image map
      ...
      throw ErrorImportExport(SCVErrIDAExpNoColor);
...
void IDAExporter::WriteHeader()
{
  ...
  idaHeader.height    = inMap->iLines();
	...
	memcpy(idaHeader.title, sTitle.scVal(), sTitle.length());
    ...
    if (rStep >= 1) 
      ...
      if (fPossible) 
        ...
        return;
			...
			throw ErrorImportExport(SCVErrTooManyValues255);
    return;
}
