 ...
 ***************************************************************/
/* ProjectionLambAzimEqualArea
   Copyright Ilwis System Development ITC
    june 1996, by Jan Hendrikse
	Last change:  JHE   7 Jul 97    3:23 pm
*/
...
ProjectionLambAzimEqualArea::ProjectionLambAzimEqualArea(const Ellipsoid& ell)
:ProjectionAzimuthal(ell)
{
  //fUseParam[pvK0] = true;
}

void ProjectionLambAzimEqualArea::Prepare()
{
  double t;
...
XY ProjectionLambAzimEqualArea::xyConv(const PhiLam& _pl) const
{
  XY xy;
  PhiLam pl = _pl;
//FORWARD(e_forward); /* ellipsoid */
  if ( !ell.fSpherical()) {
    double coslam, sinlam, q, sinb, cosb, b;
    coslam = cos(pl.Lam);
    sinlam = sin(pl.Lam);
    q = qsfn(pl.Phi);
	...
	case OBLIQ:
		...
		///break;
	case EQUIT:
		xy.y = (b = sqrt(abs(2. / (1. + cosb * coslam)))) * sinb *  ymf;
eqcon:
...
oblcon:
    	...
    	xy.x = (xy.y = sqrt(abs(2. / xy.y))) * cosphi * sin(pl.Lam);
...
PhiLam ProjectionLambAzimEqualArea::plConv(const XY& _xy) const
{
  PhiLam pl;
  XY xy = _xy;
	xy.x = xy.x / k0;
