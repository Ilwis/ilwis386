 ...
 ***************************************************************/
/* SolveParallelProjective.h
	Last change:  JH   24 Sep 99    4:49 pm
*/

#ifndef ILWSOLVEPARPROJ_H
#define ILWSOLVEPARPROJ_H
...
class IMPEXP SolveParallelProjective
{
protected:
  SolveParallelProjective();
  double rHeightAccuracy;
  int iMaxOrientCount;
  int iMaxHeightCount; 
  Map mapDTM;
	typedef double Coefs[8];
  RangeReal rrMMDTM;
  CoordBounds cbDTM;
  double rAvgScale;
  ...
  int iFindCoeffsOfParProj(int iPoints,
  ...
  Coord crdInverseOfParallelProjective(const double &rCol, const double &rRow, const double &rW);
  //int iFindProjectionCenter();
  //int iFindLightRayParameters(const double &rCol, const double &rRow);

  //double rAlpha, rBeta, rGamma; // Euler angles of 'optica axis' of sensor
