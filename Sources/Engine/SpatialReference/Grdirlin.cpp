 ...
 ***************************************************************/
/*$Log: /ILWIS 3.0/GeoReference/Grdirlin.cpp $
 * 
 * 14    28-09-04 14:16 Hendrikse
 * Extended functionality to support sub-pixel precision in the tiepoints
 * and fiducials
 * 
 * 13    28-02-03 14:30 Hendrikse
 * bool *fDoNotShowError = (bool
 * *)(getEngine()->pGetThreadLocalVar(IlwisWinApp::tlvDONOTSHOWFINDERROR))
 * ; is now used to avoid construcing a mapDTM that is not found and to
 * avoid 2 successive erro messages
 * 
 * 12    4-04-01 15:56 Koolhoven
 * added check on validity of mapDTM, if not valid throw an error
 * 
 * 11    24-10-00 11:10a Martin
 * added getobjectstructure to the objects
 * 
 * 10    23-10-00 14:07 Hendrikse
 * implemented int GeoRefDirectLinear::iMinNr() const like in
 * CoordsysDirLin
 * 
 * 9     12-10-00 15:03 Hendrikse
 * improved lay-out of addit info text
 * 
 * 8     9-12-99 16:58 Hendrikse
 * corrected AddInfo text in sFormula 
 * Approximate PixelSize (Terrain-meters/RowCol)
 * 
 * 7     13-10-99 11:44 Hendrikse
 * added mapDTM->KeepOpen(true) to speed up dtm use
 * improved SetAdditionalInfoFlag check
 * improved and extended sFormula
 * added crdGetPrincipalPoint
 * moved iFindCoeffsOfDirLin() to parent SolveDirectlinear
 * 
 * 6     2-10-99 13:08 Hendrikse
 * Removed redundant statements in Store()
 * Improved sFormula 
 * 
 * 5     22-09-99 10:01 Koolhoven
 * Protect against vr() == 0
 * 
 * 4     9/21/99 16:04 Hendrikse
 * Removed redundant write statements in Store() (already in sFormula() )
 * Added Trend-plane equation (4 lines) in sFormula() for Additional Info.
 * using the new array rTrendPlaneCoeff[3]
 * Added filling of this array in iFindNonCoplanarity()
 * 
...
GeoRefDirectLinear::GeoRefDirectLinear(const FileName& fn)
: GeoRefCTP(fn, true)
{
  ...
  ReadElement("GeoRefDirectLinear", "DTM", mapDTM);
	*fDoNotShowError = fPrev;
  init();
  ReadElement("GeoRefDirectLinear", "Height Accuracy", rHeightAccuracy);
  long iTempMax;
  ReadElement("GeoRefDirectLinear", "Max Height Iterations", iTempMax);
  iMaxHeightCount = iTempMax;
  Compute();    
}

GeoRefDirectLinear::GeoRefDirectLinear(const FileName& fn, const Map& mp, RowCol rc, bool fSubPixelPrecise)
...
void GeoRefDirectLinear::Store()
{
  ...
  GeoRefCTP::Store();
  WriteElement("GeoRef", "Type", "GeoRefDirectLinear");
  WriteElement("GeoRefDirectLinear", "DTM", mapDTM);
  WriteElement("GeoRefDirectLinear", "Height Accuracy", rHeightAccuracy);
  WriteElement("GeoRefDirectLinear", "Max Height Iterations", iMaxHeightCount);
  ObjectDependency objdep;
  objdep.Add(mapDTM);
  objdep.Store(this);
}

GeoRefDirectLinear::~GeoRefDirectLinear()
{
}

String GeoRefDirectLinear::sType() const
{
  return "GeoReference DirectLinear";
}

String GeoRefDirectLinear::sFormula() const
{
  ...
  //s &= String("\r\nPrincipal Distance in ScanRowCols: %.0f", rScanPrincDistance);
  s &= String("\r\n\r\nApproximate PixelSize (Terrain-meters/RowCol): %.2f ", rAvgScale);
	s &= String("\r\nAverage Row of active ctrl points in scanned photo: %.1f", rAvgRow);
  s &= String("\r\nAverage Col of active ctrl points in scanned photo: %.1f", rAvgCol);
	if(fSubPixelPrecision)
		s &= String("\r\nTiepoints have RowCol positions with sub-pixel precision");
  return s;
}

double GeoRefDirectLinear::rPixSize() const
{
  if (!fValid())
    return rUNDEF;
...
CoordBounds GeoRefDirectLinear::CalcCoordBounds(MinMax& mm)
{
  Coord c1, c2, c3, c4;
...
void GeoRefDirectLinear::Coord2RowCol(const Coord& crd, double& rRow, double& rCol) const
{
  ...
  double z = rZ - rAvgZ;
  double denom = (rCoeff[8] * c.x + rCoeff[9] * c.y + rCoeff[10] * z + 1);
  if (abs(denom)< EPS10) {
    rRow = rUNDEF;
    rCol = rUNDEF;
    return;
...
void GeoRefDirectLinear::RowCol2Coord(double rRow, double rCol, Coord& crd) const
{
  if (!fValid()) {
//    throw ErrorObject("Invalid GeoRef", 12345);
  ...
  const_cast<GeoRefDirectLinear *>(this)->iFindLightRayParameters(rRow, rCol);
...
/*
  // Check of Coord2RowCol transf for testing purpose taking coord rDu,rDv,0
  // (current piercing point with W==0 plane as terrainCoord):
  Coord c;
  c.x = rDu - rAvgX;
  c.y = rDv - rAvgY;
  double rZ = 0;//- rAvgZ;
  double denom = (rCoeff[8] * c.x + rCoeff[9] * c.y + rCoeff[10] * rZ + 1);
  double rTestRow = (rCoeff[0] * c.x + rCoeff[1] * c.y + 
                         rCoeff[2] * rZ + rCoeff[3]) / denom;        
  double rTestCol = (rCoeff[4] * c.x + rCoeff[5] * c.y + 
                         rCoeff[6] * rZ + rCoeff[7]) / denom;  
  rTestRow += rAvgRow;  
  rTestCol += rAvgCol; 
  if (abs(rTestRow - rRow)> 10000 || abs(rTestCol - rCol) > 10000 ){
    crd = crdUNDEF;
    return; 
  }  
  // end Check
...
bool GeoRefDirectLinear::fEqual(const IlwisObjectPtr& obj) const
{
  ...
  if (rcSize() != grp->rcSize())
    return false;
  const GeoRefDirectLinear* grc = dynamic_cast<const GeoRefDirectLinear*>(grp);
  if (0 == grc)
    return false;
  for (int i = 0; i <= 10; ++i)
...
int GeoRefDirectLinear::Compute()
{
  _fValid = false;
  if (!mapDTM.fValid())
    return -5;
  ...
  if (0 == iRes) 
    ...
    for (i = 0; i < iNr; ++i) 
      ...
      crdRowCol[i].y -= rAvgCol;
		...
		iRes = iFindCoeffsOfDirLin(iNr, crdRowCol, crdXY, crdZ, rCoeff);
		if (iRes == 0) {
...
void GeoRefDirectLinear::Rotate(bool fSwapRows, bool fSwapCols, bool fRotate)
{
  GeoRefPtr::Rotate(fSwapRows, fSwapCols, fRotate);
}

void GeoRefDirectLinear::GetDataFiles(Array<FileName>& afnDat, Array<String>* asSection, Array<String>* asEntry) const
{
...
int GeoRefDirectLinear::iMinNr() const
{
  return 6;  
}

void GeoRefDirectLinear::GetObjectStructure(ObjectStructure& os)
{
	GeoRefCTP::GetObjectStructure( os );
	if ( os.fGetAssociatedFiles() )
	{
		os.AddFile(fnObj, "GeoRefDirectLinear", "DTM");		
	}		
