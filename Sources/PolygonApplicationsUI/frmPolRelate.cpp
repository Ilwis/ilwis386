...
{
	new FormPolygonMapRelate(wnd, s.c_str());
	return -1;
}

FormPolygonMapRelate::FormPolygonMapRelate(CWnd* mw, const char* sPar)
: FormPolygonMapCreate(mw, TR("Relation to other Features")),negation(false),fsMatrix(NULL),relation(0)
{
	if (sPar)
	{
		TextInput ip(sPar);
	...
	FieldDataType *fd = new FieldDataType(root, TR("First Input map"), &sInMap1, ".mpa", true);
...
int FormPolygonMapRelate::exec()
{
	...
	String sExpr = String("PolygonMapRelate(%S,%S,%S,%S)", fnMap1.sRelativeQuoted(), fnMap2.sRelativeQuoted(),rel,negate);
