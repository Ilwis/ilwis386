...
double dbl2_distance( 	TINPoint	*point0,
						TINPoint	*point1 )
{
...
//bool ConvertPntList(	vector<TINPoint> *Pnt2List, 
//						vector<TINPoint3D> *Pnt3List )
//{
//	bool bRetCode = false;
//	int nPoints, i;
//	TINPoint3D ep3;
...
//
//
//bool ConvertPntList(	vector<TINPoint3D> *Pnt3List, 
...
int iPolygonRotDirection( vector<TINPoint> *PntList )
{
	int iRot, i;
	int nPoints = (int)PntList->size( );
	TINPoint	*work = new TINPoint[nPoints];
	if( !work ) {
...
bool bPolygonSwapRotation( 	vector<TINPoint>	*vers, 
							bool			bClosed )
{
...
int	CreateConvexHull( 	
		vector<TINPoint> *points,		// [i] input points
...
int	CreateConvexHull( 	
		...
		vector<TINPoint> *Pnt2List,	// [i] input points
		vector<int>		*convh )	// [o] convex hull, list of point numbers
{	
	...
	double	*angles = NULL;
	double dPie = PIE;
	TINPoint	ep, base_point, *Pu, *Pv, *Pw;
...
int MinDicrionaryDbl2( vector<TINPoint> *points )
{
	int	npoints = (int)points->size( );
...
int MinDicrionaryDbl2( int npoints, vector<TINPoint> *points )
{
	int	i, imin;
	double	xmin, ymin;
	TINPoint ep;
