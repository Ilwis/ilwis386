													 ...
													 map(mp), m_rIsoStartVal(rUNDEF), m_rIsoEndVal(rUNDEF), m_rIsoStepVal(rUNDEF), fEightCon(f8Con), fSmooth(fSmth), fragmentSize(frags)
{
	...
	if (!fnObj.fValid()) // 'inline' object
		objtime = objdep.tmNewest();
	for(set<double>::const_iterator cur = seq.begin(); cur != seq.end(); ++cur)
		intervals.insert(*cur);
}

void SegmentMapFromRasValueBnd::parseSequence(const String& sequence, set<double>& intervals) {
	intervals.clear();
	Array<String> parts;
	Split(sequence,parts,",");
	for(int i = 0; i < parts.size(); ++i) {
		double v = parts[i].rVal();
		if ( v == rUNDEF)
			throw ErrorObject(TR("Error in sequence of numbers"));
		intervals.insert(v);
	}
}
...
void SegmentMapFromRasValueBnd::Store()
{
	...
	WriteElement("SegmentMapFromRasValueBnd", "Sequence", seq);
...
{
	if (m_rIsoStepVal == 0 && intervals.size() == 0) // would cause "div. by zero"
		return false;

	int iNrIsoLevels = (intervals.size() == 0) ? (int)((m_rIsoEndVal - m_rIsoStartVal) / m_rIsoStepVal) : intervals.size();
	...
	for (i=0; i < iLines + 1; ++i) {
		...
		double rIsoVal = (intervals.size() == 0) ? m_rIsoStartVal : *(intervals.begin());
		set<double>::iterator isoIter = intervals.begin();
		for (iCurrentIsoLine = 0; iCurrentIsoLine < iNrIsoLevels; ++iCurrentIsoLine)
		{
