...
bool VoronoiDiagramGenerator::generateVoronoi(vector<Coord> coords, const CoordBounds& cbIn, double minDist)
{
	cleanup();
	cleanupEdges();
	int i;
	...
	for(i = 0; i< nsites; i++)
	{
		sites[i].coord.x = coords[i].x;
		sites[i].coord.y = coords[i].y;
		sites[i].sitenbr = i;
		sites[i].refcnt = 0;

		if(coords[i].x < cb.MinX())
			xmin = coords[i].x;
		else if(coords[i].x > cb.MaxX())
			xmax = coords[i].x;

		if(coords[i].y < cb.MinY())
			ymin = coords[i].y;
		else if(coords[i].y > cb.MaxY())
			ymax = coords[i].y;
...
struct Halfedge * VoronoiDiagramGenerator::ELgethash(int b)
{
	...
	/* Hash table VoronoiPoints to deleted half edge.  Patch as necessary. */
...
struct Halfedge * VoronoiDiagramGenerator::ELleftbnd(struct VoronoiPoint *p)
{
	...
	if (he==ELleftend  || (he != ELrightend && right_of(he,p)))
	{
		...
		{
			he = he -> ELright;
		} while (he!=ELrightend && right_of(he,p));	//keep going right on the list until either the end is reached, or you find the 1st edge which the VoronoiPoint
		he = he -> ELleft;				//isn't to the right of
	}
	else 							//if the VoronoiPoint is to the left of the HalfEdge, then search left for the HE just to the left of the VoronoiPoint
		do 
		{
			he = he -> ELleft;
...
/* This delete routine can't reclaim node, since VoronoiPointers from hash
...
struct Edge * VoronoiDiagramGenerator::bisect(struct Site *s1,struct	Site *s2)
{
	...
	ref(s1); 
	ref(s2);
	newedge -> ep[0] = (struct Site *) NULL; //to begin with, there are no endVoronoiPoints on the bisector - it goes to infinity
...
//create a new site where the HalfEdges el1 and el2 intersect - note that the VoronoiPoint in the argument list is not used, don't know why it's there
struct Site * VoronoiDiagramGenerator::intersect(struct Halfedge *el1, struct Halfedge *el2, struct VoronoiPoint *p)
{
	...
	//create a new site at the VoronoiPoint of intersection - this is a new vector event waiting to happen
...
void VoronoiDiagramGenerator::endVoronoiPoint(struct Edge *e,int lr,struct Site * s)
{
	e -> ep[lr] = s;
	ref(s);
...
void VoronoiDiagramGenerator::clip_line(struct Edge *e)
{
	...
	//if the distance between the two VoronoiPoints this line was created from is less than 
...
bool VoronoiDiagramGenerator::voronoi(int triangulate)
{
	...
	while(1)
	{
		...
		{	
			...
			endVoronoiPoint(lbnd->ELedge,lbnd->ELpm,v);	//set the endVoronoiPoint of the left HalfEdge to be this vector
			endVoronoiPoint(rbnd->ELedge,rbnd->ELpm,v);	//set the endVoronoiPoint of the right HalfEdge to be this vector
			ELdelete(lbnd);							//mark the lowest HE for deletion - can't delete yet because there might be VoronoiPointers to it in Hash Map	
			PQdelete(rbnd);							//remove all vertex events to do with the  right HE
			ELdelete(rbnd);							//mark the right HE for deletion - can't delete yet because there might be VoronoiPointers to it in Hash Map	
			...
			bisector = HEcreate(e, pm);				//create a HE from the Edge 'e', and make it VoronoiPoint to that edge with its ELedge field
			ELinsert(llbnd, bisector);				//insert the new bisector to the right of the left HE
			endVoronoiPoint(e, re-pm, v);					//set one endVoronoiPoint to the new edge to be the vector VoronoiPoint 'v'.
													//If the site to the left of this bisector is higher than the right
													//Site, then this endVoronoiPoint is put in position 0; otherwise in pos 1
