...
Line::Line(Color color, double rWid, LineDspType typ)
...
Line::Line(LineDspType typ, Color color, Color colorFill)
...
: ldt(ldtSymbol), rWidth(0), rDist(5),
  clr(0,0,0), clrFill(255,255,255),
  fSupportLine(false),
  smb(0)
{
...
{
  ldt = (LineDspType)rpr->iLine(iRaw);
...
String Line::sConv(LineDspType ldt)
{
  return sLDT[ldt];
}

LineDspType Line::ldtConv(const String& s)
{
  for (int i = 0; i <= ldtSymbol; ++i)
    if (s == sLDT[i])
      return (LineDspType)i;
  return ldtNone;
}

int Line::psConv(LineDspType ldt)
{
  return psLDT[ldt];
}

FieldLineTypeSimple::FieldLineTypeSimple(FormEntry* par, LineDspType* type,
                             bool fonlysimple)
...
void FieldLineTypeSimple::DrawItem(Event* ev)
{
  ...
  LineDspType type = (LineDspType)value;
  Color clr, clrFill;
