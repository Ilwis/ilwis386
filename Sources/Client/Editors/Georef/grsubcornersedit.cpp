...
void GeoRefSubMapCornersView::FillDerivedFields(FieldGroup* fgGenerRoot)
{
	...
	else 
	{
		...
		fiWidth = new FieldInt(fgGrSize, SAFUiLastLine, &m_rcSize.Row);
		fiHeight = new FieldInt(fgGrSize, SAFUiLastCol, &m_rcSize.Col);
...
int GeoRefSubMapCornersView::CheckWithSize(Event*)
{
	...
	String sGRSize(SGRRemLinesCols_ii.c_str(), iXSize, iYSize);
	SetSizeString(sGRSize);
	String sPixSize(SGRRemPixSize_f.c_str(), m_rParentPixSize);
