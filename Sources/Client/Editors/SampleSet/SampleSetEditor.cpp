...
{
  htpTopic = htpSampleEditor;
  rFactVisibleLimit = 0.6;
  String s(SSSTitleSampleSet_s.scVal(), sms->sName());
  pane->mwParent()->SetWindowText(s.scVal());
  fOk = sms->fInitStat();
  if (!fOk)
    return;
...
//  men.EnableMenuItem(ID_OPENCLOSELEGEND, fLegendAct ? MF_ENABLED : MF_GRAYED);
  ...
  ObjectInfo::ReadElement("SampleSet", "FSWindows", sms->fnObj, iFSWin);
	for (int i=0; i < iFSWin; i++) {
		...
		int iID = 0;
    ObjectInfo::ReadElement(sSection.scVal(), "ID", sms->fnObj, iID);
		if (iID <= 0)
			iID = dw->iNewBarID();
...
public:
  ...
  : FormWithDest(wPar, SSSTitleFeatureSpace)
  {
    new FieldMapFromMapList(root, SSSUiBand1, ml, b1);
    new FieldMapFromMapList(root, SSSUiBand2, ml, b2);
    SetMenHelpTopic(htpFeatureSpace);
    create();
  }
};
...
public:
  ...
  : FormWithDest(wPar, SSSTitleMergeClasses)
  {
    new FieldClass(root, SSSUiClass2, iClass2, sms->dm()->pdsrt());
    new FieldClass(root, SSSUiClass1, iClass1, sms->dm()->pdsrt());
    SetMenHelpTopic(htpMergeClass);
    create();
  }
};
...
public:
  ...
  : FormWithDest(wPar, SSSTitleDeleteClass)
  {
    new FieldClass(root, SSSUiClass, iClass, sms->dm()->pdsrt());
    SetMenHelpTopic(htpDelClass);
    create();
  }
};
...
void SampleSetEditor::OnFeatureSpace()
{
   ...
   if (frm.fOkClicked()) {
	   FeatureSpaceWindow* fsw = new FeatureSpaceWindow(sms, b1, b2);
		 MakeFSWindow(fsw,CPoint(100,100), mpv->mwParent()->iNewBarID());
	 }
}
...
void SampleSetEditor::MakeFSWindow(FeatureSpaceWindow* fsw, CPoint p, int iID)
{
  ...
  int iLower = sms->mpl()->iLower();
	...
	String s(SSSTitleFeatureSpace_ss.scVal(), b1->sName(), b2->sName());
	gb->SetWindowText(s.scVal());
...
int SampleSetEditor::Edit(const Coord& c)
{
  ...
  if (0 == PixelEditor::Edit(c,htpSmplEditorAskValue))
    return 0;
  sms->AppClass(sValue);
...
void SampleSetEditor::PreSaveState()
{
  ...
  MapWindow* mw = mpv->mwParent();
	...
	while (pos != NULL) {
      ...
      ObjectInfo::WriteElement(sSection.scVal(), "ID", sms->fnObj, gb->GetDlgCtrlID());
...
{
  String s(SEDTitleSmsEditor_s.sVal(), sms->sName());
	return s;
}
