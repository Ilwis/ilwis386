...
bool NonIlwisObjectLister::fOK(const FileName& fnFile, const String&)
{
	String sExt = fnFile.sExt;
	// Accept files without extensions
	if (sExt.length() == 0)
		return true;

	if (FileName::fIsIlwisExtension(sExt))
		return false;

	char chr = sExt[sExt.length() - 1];  // last character of extension
	switch (chr)
	{
		case '#':
		case '%':
	...
	// Now exclude data files without special characters
	if (fCIStrEqual(sExt, ".fnd") ||
		fCIStrEqual(sExt, ".isf"))
		return false;
