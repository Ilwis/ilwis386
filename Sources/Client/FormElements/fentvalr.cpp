...
FieldTimeInterval::FieldTimeInterval(FormEntry* parent, const String& sQuestion,
									 ILWIS::TimeInterval* rng, FieldDataType* _fdc, 
									 ILWIS::Time::Mode* m)	
: FieldGroup(parent), interval(rng), fdc(_fdc),duration(0), modeOld(m),ft3(0)
{
	if ( m)
		mode =	*m;
	else {
	...
	bool hasStep = false;
	if ((*interval).fValid()) {  
		if ((*interval).getStep().isValid())
			hasStep = true;
	} 
	begin = rng->getBegin();
	end = rng->getEnd();
	duration = 	rng->getStep();
	StaticTextSimple *st;
	...
	ft1 = new FieldTime(fg,"",&begin,0,mode);
	if (st)
		ft1->Align(st, AL_AFTER);
	if (0 != fdc)
		st->Align(fdc, AL_UNDER);
	ft2 = new FieldTime(fg,"",&end,0,mode);
	ft2->Align(ft1, AL_AFTER);
	if ( hasStep){
		ft3 = new FieldTime(fg,"", &duration, 0, ILWIS::Time::mDURATION);
		ft3->Align(ft2, AL_AFTER);
	}
	fg->SetIndependentPos();
}

int FieldTimeInterval::StepChangeCallBack(Event *)
{
	if (!interval->fValid())
		return 0;
	if ( ft3)
...
int FieldTimeInterval::DomainCallBack(Event *)
{
	if ( ft3)
		ft3->Hide();
	ft1->Hide();
	...
	Show();
	begin = ILWIS::Time(vr->rrMinMax().rLo());
	end = ILWIS::Time(vr->rrMinMax().rHi());
	duration = ILWIS::Duration(vr->rStep());
	ft1->SetVal(begin, mode);    
	ft2->SetVal(end, mode);
	if ( ft3)
		ft3->SetVal(duration,ILWIS::Time::mDATE);

	return 0;  
}

void FieldTimeInterval::Show()
{
	FieldGroup::Show();
	if ( ft3)
		ft3->Show();
...
FormEntry *FieldTimeInterval::CheckData() {
	ft1->StoreData();
	ft2->StoreData();
	if ( begin > end)
		return ft2;
...
void FieldTimeInterval::SetVal(const ILWIS::TimeInterval& ti)
{ 
	ILWIS::TimeInterval tiv = ti;
	if (!tiv.fValid())
		return;
	...
	if ((r > 0.1) && (floor(r)+1e-6 > r))
		...
		r = 1e-6;
	ft3->SetVal(ILWIS::Duration(rStep),ILWIS::Time::mDATE);
	if (tiv.rLo() < -1e100)
		tiv.rLo() = -1e100;
	if (tiv.rHi() > 1e100)
		tiv.rHi() = 1e100;
	...
	begin = ILWIS::Time(tiv.rLo());
	end = ILWIS::Time(tiv.rHi());
	ft1->SetVal(begin, mode);
	ft2->SetVal(end, mode);
	if ( ft3)
		ft3->SetVal(ILWIS::Duration(rStep), ILWIS::Time::mDURATION);
}

void FieldTimeInterval::StoreData()
{
	if ( ft3)
		ft3->StoreData();
	ft1->StoreData();
	ft2->StoreData();
	if (interval ) {
		*interval = ILWIS::TimeInterval(begin, end,duration);
		if ( modeOld)
			*modeOld = mode;
	}
}
...
void FieldTimeInterval::Disable()
{
	if ( ft3)
		ft3->Disable();
	ft2->Disable();
...
void FieldTimeInterval::setMode(ILWIS::Time::Mode m) {
	mode = m;
	ft1->SetVal(begin, mode);
	ft2->SetVal(end, mode);
