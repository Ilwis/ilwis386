...
CalendarCtrlCell::CalendarCtrlCell()
{
	m_parHotSpots = new CalendarCtrlFontHotSpot[42];
...
void CalendarCtrlFontInfo::CreateFont(CDC* pDC)
{
	ASSERT(pDC);
...
BEGIN_MESSAGE_MAP(CalendarCtrl, CWnd)
	//{{AFX_MSG_MAP(CalendarCtrl)
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
	ON_WM_DEVMODECHANGE()
	ON_WM_FONTCHANGE()
...
CString CalendarCtrl::CStr(long lValue)
{
	char szValue[20];
...
/////////////////////////////////////////////////////////////////////////////
// CalendarCtrl message handlers
void CalendarCtrl::SetCurrentMonthAndYear(int iMonth, int iYear)
{
...
void CalendarCtrl::SetHeaderFont(LPCTSTR lpszFont, int iSize, BOOL bBold, BOOL bItalic, BOOL bUnderline, COLORREF cColor)
{
	ASSERT(iSize > 0);
	ASSERT(iSize <= 72);
	ASSERT(lpszFont);
...
void CalendarCtrl::SetDaysOfWeekFont(LPCTSTR lpszFont, int iSize, BOOL bBold, BOOL bItalic, BOOL bUnderline, COLORREF cColor)
{
	ASSERT(iSize > 0);
	ASSERT(iSize <= 72);
	ASSERT(lpszFont);
...
void CalendarCtrl::SetDaysFont(LPCTSTR lpszFont, int iSize, BOOL bBold, BOOL bItalic, BOOL bUnderline, COLORREF cColor)
{
	ASSERT(iSize > 0);
	ASSERT(iSize <= 72);
	ASSERT(lpszFont);
...
void CalendarCtrl::SetSpecialDaysFont(LPCTSTR lpszFont, int iSize, BOOL bBold, BOOL bItalic, BOOL bUnderline, COLORREF cColor)
{
	ASSERT(iSize > 0);
	ASSERT(iSize <= 72);
	ASSERT(lpszFont);
...
// look and feel
CSize CalendarCtrl::Compute3DBorderSize()
{
...
int CalendarCtrl::ComputeTodayNoneHeight()
{
	int iReturn = 0;
	CFont* pOldFont = NULL;
	CClientDC* pDC = NULL;
...
CSize CalendarCtrl::ComputeSize()
{
	CSize szReturn(0, 0);
...
CSize CalendarCtrl::ComputeTotalSize()
{
	CSize size;
...
// create the CFont GDI objects
void CalendarCtrl::CreateFontObjects()
{
	CClientDC* pDC = NULL;
...
BOOL CalendarCtrl::OnEraseBkgnd(CDC*) 
{
	return TRUE;
}

// draw the month/year header (ie. January 2000)
int CalendarCtrl::DrawHeader(CDC &dc, int iY, int iLeftX, int iMonthRow, int iMonthCol, int iMonth, int iYear)
{
	CRect ClientRect;
...
int CalendarCtrl::DrawDaysOfWeek(CDC &dc, int iY, int iLeftX, int, int)
{
	CRect ClientRect;
	int iStartX = 0;
	int iEndX = 0;
...
// draw days of month
int CalendarCtrl::DrawDays(CDC &dc, int iY, int iLeftX, int iMonthRow, int iMonthCol, int iMonth, int iYear)
{
	CRect ClientRect;
	int iReturn = 0;
	int iStartY = iY;
	...
	for (iRow = 1; iRow <= 6; iRow++)
	{
		...
		for (int iDayOfWeek = 1; iDayOfWeek <= 7; iDayOfWeek++)
		{
				...
				(dt < dtStart && iMonthCol == 1 && iMonthRow == 1 && m_bShowNonMonthDays))
			{
				...
				{
					cBackColor = RGB(0,0,255); //::GetSysColor(COLOR_BTNFACE);
...
BOOL CalendarCtrl::IsToday(COleDateTime &dt)
{
	BOOL bReturn = FALSE;
...
int CalendarCtrl::DrawTodayButton(CDC &dc, int iY)
{
	CRect ClientRect;
	int iReturn = 0;
...
void CalendarCtrl::OnDevModeChange(LPTSTR lpDeviceName) 
{
...
void CalendarCtrl::OnFontChange() 
{
	CWnd::OnFontChange();
...
void CalendarCtrl::OnPaletteChanged(CWnd* pFocusWnd) 
{
...
void CalendarCtrl::OnSettingChange(UINT uFlags, LPCTSTR lpszSection) 
{
...
void CalendarCtrl::OnTimeChange() 
{
	CWnd::OnTimeChange();
...
void CalendarCtrl::OnSysColorChange() 
{
...
// clear all hot-spots
void CalendarCtrl::ClearHotSpots()
{
...
BOOL CalendarCtrl::IsDateSelected(COleDateTime &dt)
{
	BOOL bReturn = FALSE;
...
void CalendarCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	{
		CalendarCtrlCell* pCell = HeaderHitTest(point);
		...
		if (pCell)
		{
			...
			// create list
			m_pHeaderList = new CalendarCtrlListCtrl;
			ASSERT(m_pHeaderList);
	...
	{
		CalendarCtrlFontHotSpot* pSpot = HitTest(point);
...
void CalendarCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (m_bTracking)
	{
		ReleaseCapture();
...
void CalendarCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (m_bTracking)
	{
		CalendarCtrlFontHotSpot* pSpot = HitTest(point);
...
void CalendarCtrl::ScrollLeft(int iCount)
{
	ASSERT(iCount > 0);
	ASSERT(iCount < 100);
...
void CalendarCtrl::ScrollRight(int iCount)
{
	ASSERT(iCount > 0);
	ASSERT(iCount < 100);
...
void CalendarCtrl::FireTodayButton()
{
	COleDateTime dtNow = COleDateTime::GetCurrentTime();
...
void CalendarCtrl::FireNotifyClick()
{
	NMHDR	NotifyArea;
...
void CalendarCtrl::FireNotifyDblClick()
{
	NMHDR	NotifyArea;
...
COleDateTime CalendarCtrl::GetDate()
{
	if (m_bMultiSel)
	{
...
void CalendarCtrl::GetDateSel(COleDateTime& dtBegin, COleDateTime& dtEnd)
{
	dtBegin = m_dtSelBegin;
	dtEnd = m_dtSelEnd;
}

void CalendarCtrl::FireNoneButton()
{
	COleDateTime dtNow = COleDateTime::GetCurrentTime();
...
void CalendarCtrl::SetRowsAndColumns(int iRows, int iCols)
{
	ASSERT(iRows > 0);
	ASSERT(iCols > 0);
	ASSERT(iRows < 100);
...
CalendarCtrlFontHotSpot* CalendarCtrl::HitTest(POINT& pt)
{
	int iCell = 0;
	BOOL bFound = FALSE;
	CalendarCtrlFontHotSpot* pReturn = NULL;
...
void CalendarCtrl::SetCellHeaderPosition(int iMonthRow, int iMonthCol, RECT rect)
{
	ASSERT(iMonthRow >= 0);
	ASSERT(iMonthCol >= 0);
...
void CalendarCtrl::SetCellPosition(int iMonthRow, int iMonthCol, RECT rect)
{
	ASSERT(iMonthRow >= 0);
	ASSERT(iMonthCol >= 0);
	ASSERT(iMonthRow <= m_iRows);
...
void CalendarCtrl::SetHotSpot(int iMonthRow, int iMonthCol, int iDayCounter, COleDateTime& dt, RECT rect)
{
	ASSERT(iMonthRow >= 0);
	ASSERT(iMonthCol >= 0);
	ASSERT(iMonthRow <= m_iRows);
...
void CalendarCtrl::AllocateCells()
{
	...
	if (iCells > 0)
	{
		...
		m_parCells = new CalendarCtrlCell[iCells];
...
void CalendarCtrl::ClearSelections()
{
...
int CalendarCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CalendarCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	CalendarCtrlFontHotSpot* pSpot = HitTest(point);
...
void CalendarCtrl::AutoConfigure()
{
	if (::IsWindow(m_hWnd))
	{
...
void CalendarCtrl::SetMaxSize(SIZE size)
{
	m_szMaxSize = size;
}

void CalendarCtrl::SetDefaultMinFontSize(int iValue)
{
	ASSERT(iValue >= 0); 
	ASSERT(iValue <= 72);
...
void CalendarCtrl::SetDefaultFont(LPCTSTR lpszValue) 
{
	ASSERT(lpszValue);
...
void CalendarCtrl::SetMonthName(int iMonth, LPCTSTR lpszName) 
{
	ASSERT(iMonth > 0); 
	ASSERT(iMonth <= 12); 
	ASSERT(lpszName);
...
CString CalendarCtrl::GetMonthName(int iMonth) 
{
	ASSERT(iMonth > 0); 
	ASSERT(iMonth <= 12); 
...
void CalendarCtrl::SetDayOfWeekName(int iDayOfWeek, LPCTSTR lpszName) 
{
	ASSERT(iDayOfWeek > 0); 
	ASSERT(iDayOfWeek <= 7); 
	ASSERT(lpszName);
...
CString CalendarCtrl::GetDayOfWeekName(int iDayOfWeek) 
{
	ASSERT(iDayOfWeek > 0); 
	ASSERT(iDayOfWeek <= 7); 
...
void CalendarCtrl::SetFirstDayOfWeek(int iDayOfWeek)
{
	ASSERT(iDayOfWeek > 0); 
	ASSERT(iDayOfWeek <= 7); 
...
int CalendarCtrl::GetFirstDayOfWeek()
{
	return m_iFirstDayOfWeek;
}

void CalendarCtrl::SetDateSel(COleDateTime dtBegin, COleDateTime dtEnd)
{
...
void CalendarCtrl::SetDate(COleDateTime dt) 
{
...
BOOL CalendarCtrl::IsSpecialDate(COleDateTime &dt)
{
	if (m_pfuncCallback)
	{
		try
...
void CalendarCtrl::SetSpecialDaysCallback(funcSPECIALDATE pValue)
{
	m_pfuncCallback = pValue;
}

CalendarCtrlCell* CalendarCtrl::HeaderHitTest(POINT& point)
{
	CalendarCtrlCell* pReturn = NULL;
...
BOOL CalendarCtrl::PreTranslateMessage(MSG* pMsg)
{
...
//-----------------------------------------------------------
CalendarSelect::CalendarSelect(FormEntry* par)
  : FormEntry(par,0,true)
{
  psn->iMinWidth = psn->iWidth = 200;
  psn->iMinHeight = psn->iHeight = 180;
  calendar = NULL;
}

void CalendarSelect::create()
{
  zPoint pntFld = zPoint(psn->iPosX + 10,psn->iPosY);
  ...
  if ( tempDate == tUNDEF)
    ...
    SetDate(tempDate);
  calendar->SetRowsAndColumns(1, 1);
  CreateChildren();
}

bool CalendarSelect::fValid()
{
	if ( !calendar) return false;
	return calendar->GetSafeHwnd() != NULL;
}
...
void CalendarSelect::show(int sw)
{
  if (calendar->GetSafeHwnd( ))
    calendar->ShowWindow(sw);
}

COleDateTime CalendarSelect::GetDate() {
	if ( calendar)
