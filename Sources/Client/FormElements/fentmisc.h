...
public:
	CalendarForm(CWnd *wnd, const String& date);
...
public:
  ...
  FieldDate(FormEntry* parent, const String& sQuestion, String *tm);
  ~FieldDate();
  void SetVal(const String& data);
//  void Show(int sw);         // overriden
  FormEntry* CheckData();    // overriden
  void create();             // overriden
  void StoreData();          // overriden
...
class _export FieldTimeSimple: public FormEntry{
public:
	FieldTimeSimple(FormEntry *f);
	void create();
private:
...
public:
	FieldTime(FormEntry *f, const String& sQuestion,bool _includeDate=true);
	void create();

private:
	StaticTextSimple *st;           // text before field
    FieldTimeSimple *fldtime;
