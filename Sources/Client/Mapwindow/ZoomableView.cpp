...
void ZoomableView::wmsDrawerState(WMSMapDrawer *drw, CRect rect, Case c) {
	drw->SetDrawerState(rect, dim, c);
	iYsize = drw->iYsize;
	iXsize = drw->iXsize;
	//iXmax = drw->iXmax;
	// = drw->iYmax;
...
bool ZoomableView::wms(CRect rect, Case c) {
	WMSMapDrawer *drw = getWMSDrawer();
	if ( drw ) {
		CRect zoomRect = rect;
		if ( c == cResize) {
			if ( drw->mm2.rcMax.fUndef())
				return false;
			//iXfalseOffset = (rect.Width() - drw->mm1.width())/2;
			//iYfalseOffset = (rect.Height() - drw->mm1.height())/2;
			//drw->rct
			//return true;
		}
		wmsDrawerState(drw, zoomRect, c);
		MapCompositionDoc* mcd = (MapCompositionDoc*)GetDocument();
		mcd->initBounds(drw->mm2);
		//CalcFalseOffsets();
		if ( c != cEntire) {
			setScrollBars();
			SetDirty();
...
bool ZoomableView::wms(CPoint p, Case c) {
	WMSMapDrawer *drw = getWMSDrawer();
	if ( drw ) {
		double mult = c == cZoomIn ? 4.0 : 0.707;
		CPoint p1, p2;
		double f = (double)dim.width() / dim.height();
		p1.x = p.x - dim.width()/mult;
		p1.y = p.y - dim.width()/(mult * f);
		p2.x = p.x + dim.width()/mult;
		p2.y = p.y + dim.width()/(mult * f);
		CRect rct(p1,p2);
		wmsDrawerState(drw, rct, c);
		return true;
	}
	return false;
}

bool ZoomableView::wmsScrollbars() {
	WMSMapDrawer *drw = getWMSDrawer();
	if ( drw ) {
		Map& mp = const_cast<Map&>(drw->mpGet());
		GeoRefCornersWMS *grfwms = mp->gr()->pgWMS();
		RowCol rc = grfwms->rcSize();
		CoordBounds cbNew = grfwms->calcNewCB(drw->mm2, drw->mm1, drw->oldBounds);
		CoordBounds cbInitial = grfwms->getInitialCoordBounds();
		double fx = cbNew.width() / cbInitial.width();
		double fy = cbNew.height() / cbInitial.height();
		...
		si.nMax = 32000;
		si.nPage = fy * si.nMax ;
		si.nPos = max(0,((double)(cbInitial.cMax.y - cbNew.cMax.y) / cbInitial.height())* si.nMax) ;
		SetScrollInfo(SB_VERT, &si);
		si.nPage = fx * si.nMax;
		si.nPos = max(0,((double)(cbInitial.cMax.x - cbNew.cMax.x) / cbInitial.width())* si.nMax) ;
		SetScrollInfo(SB_HORZ, &si);

		iXsize = drw->mm2.width() / fx;
		iYsize = drw->mm2.height() / fy;
		iYpage = drw->mm2.height();
		iXpage = drw->mm2.width();
		iXmax = iXsize - iXpage;
		iYmax = iYsize - iYpage;
		iXpos = 0;
		iYpos = 0;
...
bool ZoomableView::wmsPan(int dif, bool vertical) {
	WMSMapDrawer *drw = getWMSDrawer();
	if (drw) {
		drw->oldBounds = drw->mm2;
		if (vertical) {
			drw->mm1.MinRow() = drw->mm2.MinRow() + dif;
			drw->mm1.MaxRow() = drw->mm2.MaxRow() + dif;
			drw->mm1.MinCol() = drw->mm2.MinCol();
			drw->mm1.MaxCol() = drw->mm2.MaxCol();
		} else {
			drw->mm1.MinCol() = drw->mm2.MinCol() + dif;
			drw->mm1.MaxCol() = drw->mm2.MaxCol() + dif;
			drw->mm1.MinRow() = drw->mm2.MinRow();
			drw->mm1.MaxRow() = drw->mm2.MaxRow();
		}
		drw->drawCase = cPan;
		return true;
	}
...
WMSMapDrawer *ZoomableView::getWMSDrawer() {
	MapCompositionDoc* mcd = (MapCompositionDoc*)GetDocument();
	if (!mcd)
		return NULL;
	if ( dynamic_cast<MapPaneView *>(this)) {
