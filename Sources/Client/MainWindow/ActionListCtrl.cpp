...
class ActionListCtrlDropTarget: public COleDropTarget
{
public:
	BOOL Register(ActionListCtrl* acttc)
	{
		atc = acttc;
...
BEGIN_MESSAGE_MAP(ActionListCtrl, CListCtrl)
...
{
	odt = new ActionListCtrlDropTarget;
}
...
int ActionListCtrl::Create(ActionTabs *tabs)
{
	CRect rct(0,0,0,0);
	int iRet = CListCtrl::Create(WS_CHILD | WS_VISIBLE | WS_BORDER | LVS_SINGLESEL | LVS_REPORT |
		                           LVS_SORTASCENDING | LVS_NOCOLUMNHEADER , rct, tabs, 1932);
//	SetExtendedStyle(LVS_EX_INFOTIP); -- switch off
	...
	InsertColumn(0, "Operations", LVCFMT_LEFT, 80);
  int iIndex = 0;
	SetItemCount(40);
...
void ActionListCtrl::OnDblclk(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void ActionListCtrl::OnReturn(NMHDR* pNMHDR, LRESULT* pResult) 
{
	OnDblclk(pNMHDR, pResult);
}

void ActionListCtrl::OnGetInfoTip(NMHDR* pNMHDR, LRESULT* pResult)
{
...
DROPEFFECT ActionListCtrl::OnDragEnter(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point) 
{
...
void ActionListCtrl::OnDragLeave() 
{
	// TODO: Add your specialized code here and/or call the base class
}

DROPEFFECT ActionListCtrl::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point) 
{
	return OnDragEnter(pDataObject, dwKeyState, point);
}

BOOL ActionListCtrl::OnDrop(COleDataObject* pDataObject, DROPEFFECT dropEffect, CPoint point) 
{
	if (!pDataObject->IsDataAvailable(CF_HDROP))
		return FALSE;
	int iIndex = HitTest(point);
...
void ActionListCtrl::OnContextMenu(CWnd* pWnd, CPoint point) 
{
	CPoint pt = point;
	ScreenToClient(&pt);
...
void ActionListCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{ // just here to prevent that default implementation 
	// prevents a WM_CONTEXTMENU to be sent
}

void ActionListCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
...
void ActionListCtrl::OnClick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	int iNr = GetSelectedCount();
	if (0 == iNr) return;
	POSITION pos = GetFirstSelectedItemPosition();
...
BOOL ActionListCtrl::PreTranslateMessage(MSG* pMsg)
{
