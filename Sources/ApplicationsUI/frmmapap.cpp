 ...
 ***************************************************************/
/*
// $Log: /ILWIS 3.0/ApplicationForms/frmmapap.cpp $
 * 
 * 42    9/04/06 9:42a Lichun
 * 
 * 41    7/17/06 3:00p Lichun
 * added new application for calculation of effect distance map 
 * 
 * 40    24-10-03 16:46 Willem
 * - Changed: Aggregate map form now defines the default selection for the
 * aggregation function, instead of leaving this to the FieldAggrFunc
 * 
 * 39    25-02-02 12:07 Hendrikse
 * FormMapKrigingFromRaster is again (like before 21--2-2000) derived from
 * FormMapCreate and not from FormRasterize anymore; member sMap is again
 * added as input map name string.
 * initAsk is not needed anymore I believe
 * 
 * 38    7-09-01 18:41 Koolhoven
 ...
 * 
 * 37    15-08-01 18:31 Koolhoven
 * added option to specify a GeoRef for MapGlue
 * 
 * 36    8-08-01 9:48 Koolhoven
 * adaptions to allow start with maplist as first parameter
 * 
 * 35    7-08-01 19:47 Koolhoven
 * Aggregate allows also a MapList as input
 * 
 * 34    16/03/01 12:19 Willem
 * The column selection now also allows UniqueID columns
 * 
 * 33    9-03-01 12:14 Hendrikse
 * implemented a flag fValues to en-/disable the Show/Define buttons in
 * the proper cases for Variogramsurface and KrigingfromRaster forms
 * 
 * 32    22/02/01 16:58 Willem
 * Changed the meter/pixel size callback message in RasterKriging form
 * slightly: it now displays minimum and maximum values for limiting
 * distance.
 * 
 * 31    16/02/01 17:01 Willem
 ...
 * depending on and related to the choice of units (m or pixels). Now the
 * expression generated for the cmd line shows it in the current units.
 * 
 * 28    8-11-00 17:24 Koolhoven
 * use sFullNameQuoted() on input data to ensure selection in forms
 * 
 * 27    1-11-00 14:28 Koolhoven
 * in form MapVariogramSurface now also allow bool and image maps as input
 * 
 * 26    25-09-00 15:59 Hendrikse
 * made also bool as input domain possible by means of
 * new MapListerDomainType(".mpr", dmVALUE|dmIMAGE|dmBOOL in the form
 * consructor
 * added sDomain = "value" in the exec() to force always a (changeable)
 * value domain as output
 * implemented call to default valueRange setter vrDefault(map)
 * 
 * 25    21-09-00 10:05 Hendrikse
 * computes now a valuerange default for the output of Kriging from raster
 * (in the input mapCallback)
 * 
 * 24    31-08-00 9:24a Martin
 * mapaatribute works now with histograms and rpr's  (?!)
 * 
 * 23    28/08/00 16:14 Willem
 * Made sure that all raster application forms pass map boundaries to
 * create georef form
 * 
 * 22    10-08-00 15:13 Koolhoven
 * FormMapVariogramSurface no longer asks seprately about the attribute
 * column
 * 
 * 21    8-08-00 14:56 Koolhoven
 * added spinners with FieldInts on Aggregate: GroupFactor,
 * VariogramSurface: NrLags,  KrigingFromRaster: nr pixels distance
 * improved FormMapKrigingFromraster::UnitsCallBack() if invalid map,
 ...
 * 
 * 18    5/23/00 1:53p Wind
 * MapListerDomainType instances were used/shared by several formentries,
 * and deleted by them
 * 
 * 17    5/23/00 12:45p Wind
 * - accept  raster input maps with any domain in FormMapAggr
 * - accept  raster input maps with any domain in FormMapGlue
 * 
 * 16    11-05-00 3:57p Martin
 * added 'attribute columns' nameedits
 * 
 * 15    4/27/00 3:38p Hendrikse
 * changed 'search radius' into 'limiting dist'
 * 
 * 14    28-02-00 11:56 Wind
 * adapted to changes in constructor of FieldColumn
 * 
 * 13    21-02-00 16:45 Hendrikse
 * added feMap->StoreData(); and rgUnits->StoreData();
 * added a switch on UnitChoice
 * added string costants for error messages
 * 
 * 12    3-02-00 8:43 Hendrikse
 * added 2x stRemark->SetVal(""); in FormMapAggregate::CallBack
 * 
 ...
 * included  fldsmv.h
 * added 3 Unit(edit0 callbacks and extended constructor and mapcallback
 * for KrigingFromRaster
 * 
 * 9     27-01-00 10:00 Hendrikse
 * added class _export FormMapKrigingFromRaster implementation and related
 * stuff
 * 
 * 8     14-01-00 17:27 Koolhoven
 * Include extension (and thus icon) in input map name
 * 
 * 7     1/12/99 16:27 Willem
 * Added extension to domain name string
 * 
 * 6     16-11-99 2:10p Martin
 * blocked pure virtual call
 * 
 * 5     9/13/99 12:35p Wind
 * adapted :exec functions to use of quoted file names when building an
 * expression
 * 
 * 4     13-09-99 11:15a Martin
 * Added 2.22 stuff
 * 
 * 3     9/13/99 9:34a Wind
 * support of quoted file names
 * 
 * 2     9/08/99 12:10p Wind
*/
// Revision 1.18  1998/09/16 17:33:54  Wim
// 22beta2
//
// Revision 1.17  1998/02/24 08:46:21  martin
// Attributemaps may be in different directories.
//
// Revision 1.16  1997/09/16 18:08:19  Wim
// Set correct number of bytes per pixel if not a value domain in FormAttributeMap
//
// Revision 1.15  1997-09-15 10:47:30+02  Wim
// Clear Remark and enable OK button in GlueMap when
// the new domain field is hidden
//
// Revision 1.14  1997-09-11 11:40:29+02  Wim
// Strip path from specified new domain name
//
// Revision 1.13  1997-09-04 15:28:09+02  Wim
// FormAttributeMap set ColumnCallBack also on stRemark to be sure it is called latest
//
// Revision 1.12  1997-08-29 19:43:34+02  Wim
// Do not make a FileName of an empty sName in
// FormAttributeMap::MapCallBack()
//
// Revision 1.11  1997-08-28 17:08:44+02  Wim
// ReplaceAll in FormMapGlue by default on.
//
// Revision 1.10  1997-08-26 16:06:55+02  Wim
// FormMapNumbering and FormMapGlue now use initremark()
//
// Revision 1.9  1997-08-26 14:55:00+02  Wim
// Added sNewDom with AreaNumbering and UniqueID
//
// Revision 1.8  1997-08-22 15:51:38+02  Wim
// FormMapGlue: only ask for new domain if both maps have a domain sort
//
// Revision 1.7  1997-08-20 21:24:10+02  Wim
// Changed FormMapAggregate: added callbacks to function
//
// Revision 1.6  1997-08-15 13:10:36+02  Wim
// Use only relative names for ThiessenMap in FormMapDistance
//
// Revision 1.5  1997-08-13 09:51:52+02  Wim
// AreaNumbering: allow only class, id and bool maps
//
// Revision 1.4  1997-08-04 17:55:20+02  Wim
// Corrected syntax produced by FormMapGlue
//
// Revision 1.3  1997-07-31 11:51:22+02  Wim
// Allow bool domain as source map in MapDistance
//
// Revision 1.2  1997-07-30 17:00:04+02  Wim
// Added option New Domain to create a domain instead of an internal domain for FormMapGlue
//
/* Form Map Applications excluding from Points and GeoRef change
   by Wim Koolhoven
   (c) Ilwis System Development ITC
	Last change:  WK   29 Jun 98    4:04 pm
*/
