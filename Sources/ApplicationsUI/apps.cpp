...
(*infosui).push_back(CommandHandlerUI::createCommandInfo("attribtomap","",Cmdattribtoras,TR("&Raster Operations..&Attribute to Map"),TR("Attribute to map"),"ExeMap16IcoL",".mpr",4002,TR("Create a map where a linked value < -- > Class/Id pair change the domain of a value map to class/id map")));
...
(*infosui).push_back(CommandHandlerUI::createCommandInfo("parallaxcorrection","",Cmdcorrectparallax,TR("Image &Processing..&Parallax Correction"),TR("Parallax Correction"),"ExeMap16IcoL",".mpr.mpl",4152,TR("Parallax-Correct a raster map using a DTM")));
