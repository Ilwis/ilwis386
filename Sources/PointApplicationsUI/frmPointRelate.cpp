...
{
	new FormPointMapRelate(wnd, s.scVal());
	return -1;
}

FormPointMapRelate::FormPointMapRelate(CWnd* mw, const char* sPar)
: FormPointMapCreate(mw, SAFTilteMaprelate),negation(false),fsMatrix(NULL),relation(0)
{
	if (sPar)
	{
		TextInput ip(sPar);
	...
	FieldDataType *fd = new FieldDataType(root, SAFUiFirstInput, &sInMap1, ".mpp", true);
