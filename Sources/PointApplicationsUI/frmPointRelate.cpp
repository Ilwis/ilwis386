 ...
 ***************************************************************/
/*
// $Log: /ILWIS 3.0/ApplicationForms/frmPointMapRelate.cpp $
 * 
 ...
 * Always a DomainPointMapRelate will be used
 * 
 ...
 * Added a field to enter the domain prefix for PointMapRelate operations for
 * pointmap, segmentmap and Pointmap
 * 
...
// Added sNewDom with AreaNumbering and PointMapRelate
//
...
LRESULT Cmdpointmaprelate(CWnd *wnd, const String& s)
{
	new FormPointMapRelate(wnd, s.c_str());
	return -1;
}

FormPointMapRelate::FormPointMapRelate(CWnd* mw, const char* sPar)
: FormPointMapCreate(mw, TR("Relation to other Features")),negation(false),fsMatrix(NULL),relation(0)
{
	if (sPar)
	{
		TextInput ip(sPar);
	...
	FieldDataType *fd = new FieldDataType(root, TR("First Input map"), &sInMap1, ".mpp", true);
	...
	fsRelation->SetCallBack((NotifyProc)&FormPointMapRelate::SetPattern);
	...
	fd = new FieldDataType(root, TR("Second Input map"), &sInMap2, ".mpa.mps.mpp", true);
	...
	//SetAppHelpTopic(htpGenMapPointMapRelate);
	create();
}

FormEntry *FormPointMapRelate::CheckData() {
	if ( relation == 7) {
		if ( pattern.size() != 9)
			return fsMatrix;
		pattern.toUpper();
...
int FormPointMapRelate::SetPattern(Event *ev) {
