 ...
 ***************************************************************/
/*
// $Log: /ILWIS 3.0/ApplicationForms/frmPointMapIntersection.cpp $
 * 
 ...
 * Always a DomainPointMapIntersection will be used
 * 
 ...
 * Added a field to enter the domain prefix for PointMapIntersection operations for
 * pointmap, Pointmap and Pointmap
 * 
...
// Added sNewDom with AreaNumbering and PointMapIntersection
//
...
: FormPointMapCreate(mw, TR("Create a Intersection map of Points"))
{
	...
	//SetAppHelpTopic(htpGenMapPointMapIntersection);
	create();
}

FormEntry *FormPointMapIntersection::CheckData() {
	return NULL;
}

int FormPointMapIntersection::exec()
{
	FormPointMapCreate::exec();
	FileName fnMap1(sInMap1);
	FileName fnMap2(sInMap2);
	String sExpr = String("PointMapIntersect(%S,%S)", fnMap1.sRelativeQuoted(), fnMap2.sRelativeQuoted());
