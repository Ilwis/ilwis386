...
{
	new FormPointMapCross(wnd, s.c_str());
	return -1;
}

FormPointMapCross::FormPointMapCross(CWnd* mw, const char* sPar)
: FormPointMapCreate(mw, TR("Pointmap Cross"))
{
	if (sPar)
	{
		TextInput ip(sPar);
	...
	new FieldDataType(root, TR("Pointmap"), &sInMap1, ".mpp", true);
	new FieldDataType(root, TR("Maplist"), &sInMap2, ".mpl", true);
...
FormEntry *FormPointMapCross::CheckData() {
	return NULL;
}

int FormPointMapCross::exec()
{
