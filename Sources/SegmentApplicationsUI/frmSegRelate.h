 ...
 ***************************************************************/
/*
// $Log: /ILWIS 3.0/ApplicationForms/frmSegmentMapRelate.h $
 * 
...
class _export FormSegmentMapRelate: public FormSegmentMapCreate
{
public:
	FormSegmentMapRelate(CWnd* mw, const char* sPar);
protected:

private:  
	int exec();
	int SetPattern(Event *ev);
	int SetExtension(Event *ev);
	FormEntry *CheckData();
	FieldOneSelectString *fsRelation;
	FieldString *fsMatrix;
	FieldDataType *fInput1;
	FieldDataType *fInput2;
