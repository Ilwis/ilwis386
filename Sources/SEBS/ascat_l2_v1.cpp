...
#define IPR_SIZE     27
#define SMO_SIZE   5830
#define SMR_SIZE   11350
...
#define THROW_ASCAT_L2 throwpos3(__FILE__,__LINE__)

///////////////////////////////////
// function to throw a message
///////////////////////////////////

void throwpos3( const char *file, int line )
{
 string msg;
...
ascat_l2v1_file::ascat_l2v1_file()
{
 mdr = NULL;
 fp = NULL;
}
...
int ascat_l2v1_file::get_sat( )
{
 return sat;
}

int ascat_l2v1_file::get_fmt( )
{
 return fmt;
}

int ascat_l2v1_file::get_nn( )
{
 return nn;
}

int ascat_l2v1_file::get_nr( )
{
 return nr;
}
...
int ascat_l2v1_file::check( string fname, string *ans )
{
 string instr,prod;
...
int ascat_l2v1_file::open(string fname, int *nr0, int *nn0 )
{
 unsigned char x[MPHR_SIZE];
...
static void get_smo_node( unsigned char *x, int i, int swath, ascat_node *b )
{
 ...
 get_time( x, &b->tm );
 get_ushort( x, 28, 1.0e-2, &b->track );
 get_int( x, 156 + i*4, 1.0e-6, &b->lat );
 get_int( x, 324 + i*4, 1.0e-6, &b->lon );
 get_int( x, 744 + i*12, 1.0e-6, &b->s0, &b->s1, &b->s2 );
 get_ushort( x, 1248 + i*6, 1.0e-4, &b->kp0, &b->kp1, &b->kp2 );
 get_ushort( x, 1500 + i*6, 1.0e-2, &b->t0, &b->t1, &b->t2 );
 get_short( x, 1752 + i*6, 1.0e-2, &b->a0, &b->a1, &b->a2 );
 get_uchar( x, 2004 + i*3, &b->fkp0, &b->fkp1, &b->fkp2 );
 get_uchar( x, 2130 + i*3, &b->fuse0, &b->fuse1, &b->fuse2 );
 get_ushort( x, 3768 + i*6, 1.0e-3, &b->fland0, &b->fland1, &b->fland2 );
 get_ushort( x, 4024 + i*2, 1.0e-2, &b->sm);
 get_ushort( x, 4108 + i*2, 1.0e-2, &b->sm_error);
 get_int( x, 4192 + i*4, 1.0e-6, &b->sigma40);
 get_int( x, 4360 + i*4, 1.0e-6, &b->sigma40_error);
 get_int( x, 4528 + i*4, 1.0e-6, &b->slope40);
 get_int( x, 4696 + i*4, 1.0e-6, &b->sigma40_error);
 //get_uint( x, 6018 + i*4, 1.0e-6, &b->sm_sensitivity);
 get_int( x, 5032 + i*4, 1.0e-6, &b->dry_backscatter);
 get_int( x, 5200 + i*4, 1.0e-6, &b->wet_backscatter);
 get_ushort( x, 5368 + i*2, 1, &b->sm_mean);
 get_uchar( x, 5452 + i, &b->frainfall);
 get_uchar( x, 5494 + i, &b->fcorrection);
 get_ushort(x, 5536 + i * 2, &b->fprocessing);
 get_uchar( x, 5620 + i, &b->fquality);
 get_uchar( x, 5662 + i, &b->fsnow);
 get_uchar( x, 5704 + i, &b->ffrozen);
 get_uchar( x, 5746 + i, &b->fwetland);
 get_uchar( x, 5788 + i, &b->ftcomplexity);
}
...
static void get_smr_node( unsigned char *x, int i, int swath, ascat_node *b )
{
 ...
 get_time( x, &b->tm );
 get_ushort( x, 28, 1.0e-2, &b->track );
 get_int( x, 276 + i*4, 1.0e-6, &b->lat );
 get_int( x, 604 + i*4, 1.0e-6, &b->lon );
 get_int( x, 1424 + i*12, 1.0e-6, &b->s0, &b->s1, &b->s2 );
 get_ushort( x, 2408 + i*6, 1.0e-4, &b->kp0, &b->kp1, &b->kp2 );
 get_ushort( x, 2900 + i*6, 1.0e-2, &b->t0, &b->t1, &b->t2 );
 get_short( x, 3392 + i*6, 1.0e-2, &b->a0, &b->a1, &b->a2 );
 get_uchar( x, 3884 + i*3, &b->fkp0, &b->fkp1, &b->fkp2 );
 get_uchar( x, 4130 + i*3, &b->fuse0, &b->fuse1, &b->fuse2 );
 get_ushort( x, 7328 + i*6, 1.0e-3, &b->fland0, &b->fland1, &b->fland2 );
 get_ushort( x, 7824 + i*2, 1.0e-2, &b->sm);
 get_ushort( x, 7988 + i*2, 1.0e-2, &b->sm_error);
 get_int( x, 8152 + i*4, 1.0e-6, &b->sigma40);
 get_int( x, 8400 + i*4, 1.0e-6, &b->sigma40_error);
 get_int( x, 8808 + i*4, 1.0e-6, &b->slope40);
 get_int( x, 9136 + i*4, 1.0e-6, &b->sigma40_error);
 //get_uint( x, 6018 + i*4, 1.0e-6, &b->sm_sensitivity);
 get_int( x, 9792 + i*4, 1.0e-6, &b->dry_backscatter);
 get_int( x, 10120 + i*4, 1.0e-6, &b->wet_backscatter);
 get_ushort( x, 10448 + i*2, 1.0e-2, &b->sm_mean);
 get_uchar( x, 10612 + i, &b->frainfall);
 get_uchar( x, 10692 + i, &b->fcorrection);
 get_ushort(x,10776 + i*2,&b->fprocessing);
 get_uchar( x, 10940 + i,&b->fquality);
 get_uchar( x, 11022 + i,&b->fsnow);
 get_uchar( x, 11104 + i,&b->ffrozen);
 get_uchar( x, 11186 + i,&b->fwetland);
 get_uchar( x, 11268 + i,&b->ftcomplexity);
}

void ascat_l2v1_file::get_node( int i, int swath, ascat_node *b )
{
 switch( nn )
  {
   case NODES_SMO :
