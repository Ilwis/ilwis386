...
ARY TRSACT_file_count_old ( char *fname, int *Tnum ){
...
ARY TRSACT_file_count (vector<vector<int>> & items){
  ARY E;
  //int Tnum = 0; /* # transactions */
  ARY_init ( &E, sizeof(int) );

  int rows = items.size();
  for (int row = 0; row < rows; ++row) {
	  int cols = items[row].size();
	  for (int col = 0; col < cols; ++col) {
		  int item = items[row][col];
		  ARY_exp_const ( &E, item, 0 );
		  ((int *)E.h)[item]++;
	  }
  }
...
/***********************************/
int *TRSACT_load_old ( char *fname, ARY *T, int th ){
  QUEUE *Q;
  ARY E;
  ...
  E = TRSACT_file_count_old ( fname, &Tnum );
  int *Eq = (int*)(E.h), *b;
  LCM_Trsact_num = Tnum;
...
int *TRSACT_load ( vector<vector<int>> & items, ARY *T, int th ){
  QUEUE *Q;
  ARY E;
  int Tnum, Enum, bnum; /* #transactions, max item, #all freq items */
  int i;
  ...
  int fc=TRSACT_BUFSIZ, p= TRSACT_BUFSIZ;
  int *perm;
  E = TRSACT_file_count ( items );
  int *Eq = (int*)(E.h), *b;
  LCM_Trsact_num = items.size();
  perm = TRSACT_perm_freq_item ( &E, th, &bnum, &Enum );

  ARY_init ( T, sizeof(QUEUE) );
  ARY_exp ( T, LCM_Trsact_num + 2 );
  Q = (QUEUE*)(T->h);
  ...
  for (int row = 0; row < rows; ++row) {
	  ...
	  if (j != i) {
		...
		QUEUE_sort_ ( &Q[Tnum] );
		b = Q[Tnum].q;
		for ( int item=0,item2=1 ; item2<Q[Tnum].t ; item2++ ){
		  if ( b[item] != b[item2] ){
			item++;
			if ( item < item2 ) b[item]=b[item2];
		  }
...
void LCM_fstore_solution(vector<vector<int>> & solutions, vector<int> & count){
  int i, e;
  vector<int> solution;
  QUEUE_FE_LOOP_ ( LCM_itemset, i, e ){
...
void LCM_store_solution(vector<vector<int>> & solutions, vector<int> & count) {
#ifdef LCM_OVERTIME_END
  if ( time(NULL)-LCM_start_time > LCM_maximum_time ){
    printf ("time over\n"); return;
  }
  ...
  LCM_sc[s]++;
  LCM_counter++;
  LCM_fstore_solution(solutions, count);
}
...
/*************************************************************************/
int LCM_init_old ( int argc, char *argv[] ){
  int i, n=0, m;
  QUEUE *Q;
  ...
  LCM_perm = TRSACT_load_old ( argv[1], &LCM_Trsact, LCM_th );
...
int LCM_init ( int th, vector<vector<int>> & items ){
  int i, n=0, m;
  QUEUE *Q;
  ...
  LCM_print_flag |= 1;
  //LCM_perm = TRSACT_load_old ( argv[1], &LCM_Trsact, LCM_th );
  LCM_perm = TRSACT_load ( items, &LCM_Trsact, LCM_th );
  LCM_Eend = LCM_Trsact.dellist;
  LCM_buf = ((QUEUE *)(LCM_Trsact.h))->q;
  if ( LCM_Trsact.num == 0 ){
    printf ("there is no frequent itemset\n");
    ARY_end ( &LCM_Trsact );
    free2 ( LCM_perm );
    return (0);
  }
