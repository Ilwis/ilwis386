...
/*
** The original source files (lcm21.zip) can be downloaded here: http://research.nii.ac.jp/~uno/code/lcm21.zip            **
** The three files that deliver the fim_maximal routine (lcm_max.c, trsact.c and lib_e.c) were modified.                  **
** The following enhancements were made:                                                                                  **
** 1. Resolved compilation issues on compilers other than gcc; the code is made compatible with both gcc and microsoft cl **
** 2. The code is made C++ compatible                                                                                     **
** 3. The code was changed to work with in-memory arrays instead of input/output text-files.                              **
**    This avoids writing/reading intermediate files for each call to the lcm routine.                                    **
** 4. Resolved some memory leaks                                                                                          **
** 5. Resolved bad addressing in void* arrays                                                                             **
*/
...
int LCMmax_freq_calc( ARY *T, QUEUE *occ, ARY *O, QUEUE *jump, QUEUE_INT max_item ){
  ...
  QUEUE *Q = (QUEUE*)(T->h), *QQ = (QUEUE*)(O->h);
...
/*************************************************************************/
int LCMmax_iter(ARY *T,ARY *O,int item, int *pm, int *pm2, int prv, QUEUE *jump, vector<vector<int>> & solutions, vector<int> & count){
  ARY TT, OO;
  QUEUE *occ = (QUEUE*)(O->h), *oocc, jump2;
  int i, ii, e, ee, n, nn, addtail=LCM_add.t, js=LCM_jump.s, *perm;
  int qt = LCM_Qtmp.t, flag=-1, cc=0, itemt, itemt2, itemt22, ff=0;
  QUEUE_INT *q;
  ...
  if ( QUEUE_LENGTH(*jump)==0 ){   /* no freq. item => maximal  */
    // LCM_print_solution_ ();
    LCM_store_solution (solutions, count);
    goto END2;
  }
...
/******************************************************/
  ...
  if ( ii<4 ){
    ...
    do {
      ...
      LCMmax_iter ( &TT, O, i, pm, NULL, item, &jump2, solutions, count );
      occ[i].t = occ[i].end = 0;
    } while ( jump->t > jump->s );
    goto END1;
  }
  ...
  LCMmax_iter ( &TT, O, ee, pm, NULL, item, &jump2, solutions, count );
  occ[ee].t = 0;
  ...
  for ( i=0 ; i<prv-1 ; i++ ){
    LCM_itemset2.t = itemt;
    if ( i>=itemt2-itemt-1 ){
      LCMmax_iter(&TT, &OO, i, perm, &perm[tt], prv, &jump2, solutions, count);
...
/*************************************************************************/
int LCMmax_first_iter( int item, vector<vector<int>> & solutions, vector<int> & count ){
  ARY TT, OO, *T = &LCM_Trsact;
  QUEUE *occ = LCM_occ, *oocc, jump2;
  int i, ii, e, ee, n, addtail=LCM_add.t, js=LCM_jump.s, *perm;
  int qt = LCM_Qtmp.t, flag=-1, cc=0, itemt, itemt2, itemt22, ff=0;
  QUEUE_INT *q;
...
/******************************************************/
  ...
  if ( ii<4 ){
    ...
    do {
      ...
      LCM_itemset2.t = itemt;
      LCMmax_iter ( &TT, &LCM_Occ, i, LCM_perm, NULL, item, &jump2, solutions, count );
      occ[i].t = occ[i].end = 0;
  ...
  LCMmax_iter ( &TT, &LCM_Occ, ee, LCM_perm, NULL, item, &jump2, solutions, count );
  occ[ee].t = 0;
...
/*************************************************************************/
void LCMmax ( int n, vector<vector<int>> & solutions, vector<int> & count ){
  int i;
  ...
  for ( i=0 ; i<LCM_Eend ; i++ ){
    LCM_itemset2.t = 0;
    LCMmax_first_iter ( i, solutions, count );
...
void doLCM(int th, vector<vector<int>> & items, vector<vector<int>> & solutions, vector<int> & counts)
{
  int n = LCM_init ( th, items );
  LCMmax ( n, solutions, counts);
  LCM_end ();
  ARY_end ( &LCM_Trsact );
}
