...
(*infosui).push_back(CommandHandlerUI::createCommandInfo("run '" + hyrdoDemBase + "dem_visualization'.isl","",Cmdrun,TR("&DEM hydro-processing@01..DEM Visualization"),TR("DEM Visualization"),"ExeMap16IcoL",".mpr",8070,TR("Run script to calculate colour shadow map")));
...
(*infosui).push_back(CommandHandlerUI::createCommandInfo("run '" + hyrdoDemBase + "Compound_Parameter_Extraction'.isl","",Cmdrun,TR("&DEM hydro-processing@15.&Compound Parameter Extraction.Compound Index Calculation"),TR("Compound Index Calculation"),"ExeMap16IcoL","",8071,TR("Compound Index Calculation")));
(*infosui).push_back(CommandHandlerUI::createCommandInfo("hortonplots","",Cmdhortonplots,TR("&DEM hydro-processing@16.&Statistical Parameter Extraction.Horton Statistics"),TR("Horton Statistics"),"ExeTbl16IcoL",".mpr",8057,TR("Calculate parameters for Horton plots")));
(*infosui).push_back(CommandHandlerUI::createCommandInfo("run '" + hyrdoDemBase + "aggregate_statistics'.isl","",Cmdrun ,TR("&DEM hydro-processing@17.&Statistical Parameter Extraction.Aggregate Statistics"),TR("Aggregate Statistics "),"ExeTbl16IcoL",".tbl",8072,TR("Run script to calculate aggregate statistics")));
(*infosui).push_back(CommandHandlerUI::createCommandInfo("run '" + hyrdoDemBase + "cumulative_hypsometric_curve'.isl","",Cmdrun,TR("&DEM hydro-processing@18.&Statistical Parameter Extraction.Cumulative Hypsometric Curve"),TR("Cumulative Hypsometric Curve"),"ExeTbl16IcoL","",8073,TR("Run script to construct cumulative hypsometric curve for a given catchment")));
(*infosui).push_back(CommandHandlerUI::createCommandInfo("run '" + hyrdoDemBase + "class_coverage_statistics'.isl","",Cmdrun,TR("&DEM hydro-processing@19.&Statistical Parameter Extraction.Class Coverage Statistics"),TR("Class Coverage Statistics"),"ExeTbl16IcoL","",8074,TR("Run script to obtain the percentage coverage of each class on a selected catchment")));
