...
#include "DrawersUI\AnimationControlTool.h"
...
int AnimationControl::stop(Event  *ev) {
	AnimationDrawer *andr = (AnimationDrawer *)drw;
	view->GetDocument()->mpvGetView()->KillTimer(andr->getTimerId());
...
int AnimationControl::run(Event  *ev) {
	...
	if ( andr->getTimerId() != iUNDEF) {
		...
		int timeIdC = AnimationDrawer::getTimerIdCounter();
		andr->setTimerId(timeIdC++);
	}
	if ( andr->getUseTime()) {
		view->GetDocument()->mpvGetView()->SetTimer(andr->getTimerId(),REAL_TIME_INTERVAL,0);
		this->SetTimer(andr->getTimerId(),REAL_TIME_INTERVAL,0);
	} else {
		view->GetDocument()->mpvGetView()->SetTimer(andr->getTimerId(),andr->getInterval() * 1000.0,0);
